"""RAG system orchestrator for document retrieval and AI generation.

This module coordinates all components of the RAG chatbot including
document processing, vector storage, AI generation, and session management.
"""
...
class RAGSystem:
    ...
    def add_course_document(self, file_path: str) -> Tuple[Optional[Course], int]:
        ...
        Returns:
            Tuple of (Course object or None if error, number of chunks)
        """
        try:
        ...
        for file_name in os.listdir(folder_path):
            ...
            if os.path.isfile(file_path) and file_name.lower().endswith(
                ...
                try:
                    ...
                    # Process to get course ID, add only if new
                    ...
                    if course and course.title not in existing_course_titles:
                        # This is a new course - add to vector store
                        self.vector_store.add_course_metadata(course)
                        self.vector_store.add_course_content(course_chunks)
                        total_courses += 1
                        ...
                        print(
                            f"Added new course: {course.title} ({chunk_count} chunks)"
                        )
